-- Purpose: Defines the Supabase database schema for the BALIORA app without any Base44-specific table names.
-- Used by: Supabase SQL editor / migration runner before app data is moved over.
-- Main dependencies: Postgres on Supabase, pgcrypto for UUID generation, and optional auth.jwt() for role checks.
-- Public objects: app_users, villa_listings, booking_inquiries, owner_inquiries, villa_assessments, villa_owners, blog_posts, faqs, testimonials, website_settings, activity_logs.
-- Side effects: Creates tables, indexes, updated_at triggers, and row-level security policies.
...
create or replace function public.is_public_editor()
returns boolean
language sql
stable
security definer
...
create table if not exists public.booking_inquiries (
  id uuid primary key default gen_random_uuid(),
  villa_id uuid references public.villa_listings(id) on delete set null,
...
create table if not exists public.villa_assessments (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists villa_listings_status_idx on public.villa_listings (status);
create index if not exists villa_listings_slug_idx on public.villa_listings (slug);
create index if not exists booking_inquiries_status_idx on public.booking_inquiries (status);
create index if not exists owner_inquiries_status_idx on public.owner_inquiries (status);
create index if not exists villa_assessments_status_idx on public.villa_assessments (status);
create index if not exists villa_owners_status_idx on public.villa_owners (status);
...
create index if not exists website_settings_category_idx on public.website_settings (category);
...
drop trigger if exists set_updated_at_villa_listings on public.villa_listings;
create trigger set_updated_at_villa_listings
before update on public.villa_listings
for each row execute function public.set_updated_at();

drop trigger if exists set_updated_at_booking_inquiries on public.booking_inquiries;
create trigger set_updated_at_booking_inquiries
before update on public.booking_inquiries
for each row execute function public.set_updated_at();

drop trigger if exists set_updated_at_owner_inquiries on public.owner_inquiries;
create trigger set_updated_at_owner_inquiries
before update on public.owner_inquiries
for each row execute function public.set_updated_at();

drop trigger if exists set_updated_at_villa_assessments on public.villa_assessments;
create trigger set_updated_at_villa_assessments
before update on public.villa_assessments
for each row execute function public.set_updated_at();

drop trigger if exists set_updated_at_villa_owners on public.villa_owners;
create trigger set_updated_at_villa_owners
before update on public.villa_owners
for each row execute function public.set_updated_at();
...
drop trigger if exists set_updated_at_website_settings on public.website_settings;
...
alter table public.villa_assessments enable row level security;
...
drop policy if exists "Public can read villas" on public.villa_listings;
create policy "Public can read villas"
  on public.villa_listings
  for select
  using (status = 'available' or public.is_admin_user());

drop policy if exists "Admins manage villas" on public.villa_listings;
...
drop policy if exists "Anyone can submit booking inquiries" on public.booking_inquiries;
create policy "Anyone can submit booking inquiries"
  on public.booking_inquiries
  for insert
  with check (true);

drop policy if exists "Admins manage booking inquiries" on public.booking_inquiries;
create policy "Admins manage booking inquiries"
  on public.booking_inquiries
  for select
  using (public.is_admin_user());

drop policy if exists "Admins update booking inquiries" on public.booking_inquiries;
...
drop policy if exists "Admins delete booking inquiries" on public.booking_inquiries;
create policy "Admins delete booking inquiries"
  on public.booking_inquiries
  for delete
  using (public.is_admin_user());

drop policy if exists "Anyone can submit owner inquiries" on public.owner_inquiries;
create policy "Anyone can submit owner inquiries"
  on public.owner_inquiries
  for insert
  with check (true);

drop policy if exists "Admins manage owner inquiries" on public.owner_inquiries;
create policy "Admins manage owner inquiries"
  on public.owner_inquiries
  for select
  using (public.is_admin_user());

drop policy if exists "Admins update owner inquiries" on public.owner_inquiries;
...
drop policy if exists "Admins delete owner inquiries" on public.owner_inquiries;
create policy "Admins delete owner inquiries"
  on public.owner_inquiries
  for delete
  using (public.is_admin_user());

drop policy if exists "Anyone can submit villa assessments" on public.villa_assessments;
create policy "Anyone can submit villa assessments"
  on public.villa_assessments
  for insert
  with check (true);

drop policy if exists "Admins manage villa assessments" on public.villa_assessments;
create policy "Admins manage villa assessments"
  on public.villa_assessments
  for select
  using (public.is_admin_user());

drop policy if exists "Admins update villa assessments" on public.villa_assessments;
...
drop policy if exists "Admins delete villa assessments" on public.villa_assessments;
create policy "Admins delete villa assessments"
  on public.villa_assessments
  for delete
  using (public.is_admin_user());

drop policy if exists "Public can read owners" on public.villa_owners;
create policy "Public can read owners"
  on public.villa_owners
  for select
  using (public.is_admin_user());

drop policy if exists "Admins manage owners" on public.villa_owners;
